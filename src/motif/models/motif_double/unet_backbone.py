...
from motif.models.motif_double.self_attention import (
    SeparateWindowedValuesCoordinatesAttention,
)
from motif.models.motif_double.small_layers import FeedForward
...
class MultisourceGeneralBackboneUNet(nn.Module):
    ...
    def __init__(
        ...
        for stage_idx, n_blocks in reversed(list(enumerate(n_blocks_per_stage[:-1]))):
            if stage_idx == 0:
                # For the last stage, we don't upsample the coordinates or conditionings
                # as they won't be used anymore.
                splitting = MultiSourcePatchSplitting(values_dim)
            else:
    ...
    def forward(self, x):
        ...
        for splitting, stage_blocks in self.upsampling_blocks:
            ...
            for src in x.keys():
                ...
                upsampled_coords = x[src].get("coords", None)
                ...
                if upsampled_values.shape[1:-1] != skip_values.shape[1:-1]:
                    ...
                    if upsampled_coords is not None:
...
class AdapativeConditionalNormalization(nn.Module):
    ...
    def __init__(
        ...
        if self.modulate_coords:
            ...
            self.coords_cond_proj = nn.Sequential(nn.SiLU(), nn.Linear(cond_dim, cond_proj_dim))
            # Initialize the weights of the conditional normalization to zero (no effect)
