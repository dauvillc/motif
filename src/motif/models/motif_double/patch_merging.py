...
class MultiSourcePatchSplitting(nn.Module):
    ...
    def __init__(self, values_dim, coords_dim=None, cond_dim=None):
        """Args:
        values_dim (int): Dimension of the values to be split.
        coords_dim (int, optional): Dimension of the coordinates to be split.
            If None, coordinates are not split.
        cond_dim (int, optional): Dimension of the conditioning to be split.
            If None, conditioning is not split.
        """
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        Returns:
            dict: Dictionary of outputs, such that
                outputs[(source_name, index)] contains the keys "values", "coords"
                and "conditioning". If coordinates or conditioning were not provided
                during initialization, they will not be present in the outputs.
        """
        ...
        for source, src_data in inputs.items():
            ...
            if self.coords_dim is not None:
                ...
                outputs[source]["coords"] = split_coords
            if self.cond_dim is not None:
                split_conditioning = self.conditioning_splitting(conditioning)
                outputs[source]["conditioning"] = split_conditioning
