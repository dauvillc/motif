...
class MultisourcesWindowedCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        # for each head.
        self.alpha = nn.Parameter(torch.ones(num_heads).view(1, num_heads, 1, 1))
    ...
    def forward(self, inputs: SourceEmbeddingDict) -> MultisourceTensor:
        ...
        for src, x_src in inputs.items():
            ...
            f_qk = rearrange(f_qk, "b Wh Ww (H e) -> b H (Wh Ww) e", H=self.num_heads)
            c_qk = rearrange(c_qk, "b Wh Ww (H e) -> b H (Wh Ww) e", H=self.num_heads)
        ...
        f_q = torch.cat(list(f_queries.values()), dim=-2)  # (B, H, N, Dh)
        f_k = torch.cat(list(f_keys.values()), dim=-2)  # (B, H, N, Dh)
        ...
        reweighted_values = attn_weights @ values  # (B, H, N, Dh * n)
        ...
        y = self.v_back_proj(reweighted_values)  # (B, N, n, Dv)
        # Split back into the sources
        y = torch.split(y, n_windows, dim=1)
