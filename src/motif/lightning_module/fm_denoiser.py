...
class MultisourceFlowMatchingDenoiser(MultisourceAbstractReconstructor):
    ...
    def __init__(
        ...
        regularization_max_epochs=20,
        regularization_min_weight=0.0,
        ...
        **kwargs,
        ...
        Args:
            ...
            predict_residuals (bool): If True, the model predicts the residual
                from the deterministic model, instead of the actual values.
            ...
            regularization_loss_weight (float): Weight of the regularization loss.
            regularization_max_epochs (int): Number of epochs over which the regularization
                weight will linearly decrease from its initial value to the minimum value.
            regularization_min_weight (float): Minimum weight of the regularization loss.
            regularization_alpha (float): Alpha parameter of the regularization loss.
        ...
        # Regularization loss parameters
        if regularization_loss_weight > 0.0 and not self.use_det_model:
            raise ValueError(
                "If using a regularization loss, a deterministic model must be provided."
            )
        self.reg_lambda_max = regularization_loss_weight
        self.reg_lambda = self.reg_lambda_max
        self.reg_max_epochs = regularization_max_epochs
        self.reg_lambda_min = regularization_min_weight
        self.reg_alpha = regularization_alpha
    ...
    def compute_loss(self, pred, batch, masked_batch, path_samples, uncond_pred=None):
        ...
        y_true = {
            source_index_pair: path_samples[source_index_pair].x_1
        ...
        # Compute the MSE between the true and predicted x_1 for each source
        losses = {}
        for source_index_pair in pred:
            # Compute the pointwise loss for each source.
            pred_src = pred[source_index_pair]
            true_src = y_true[source_index_pair]
            source_loss = (pred_src - true_src).pow(2)

            # Optional: compute the regularization term.
            if uncond_pred is not None and self.reg_lambda > 0.0:
                uncond_src = uncond_pred[source_index_pair].detach()  # Stop gradients
                mean_pred = masked_batch[source_index_pair]["pred_mean"]
                det_error = (mean_pred - true_src).pow(2)
                pred_dist = (pred_src - uncond_src).pow(2)
                reg_loss = (1 - (pred_dist / (det_error + 1e-6))).clamp(min=0).pow(self.reg_alpha)
                source_loss = source_loss + self.reg_lambda * reg_loss
            ...
            for k in range(n_realizations_per_sample):
                ...
                def vf_func(x_t, t):
                    ...
                    if self.cfg_scale:
                        ...
                        x1 = self.forward(self.to_double_batch(batch_t, uncond_batch))
                        # Split the conditional and unconditional batches and apply the
                        # guidance scale.
                        half_batch = next(iter(x1.values())).shape[0] // 2
                        for source_index_pair in x1:
                            cond = x1[source_index_pair][:half_batch]
                            uncond = x1[source_index_pair][half_batch:]
                            # The interpretation of the guidance scale depends on the type of CFG.
                            c = self.cfg_scale
                            new_x1 = (1 + c) * cond - c * uncond
                            new_x1 = uncond
                            x1[source_index_pair] = new_x1
                    else:
                        x1 = self.forward(batch_t)

                    # Compute the velocity fields from the predicted x_1
                    vf = {}
                    for source_index_pair, data in batch_t.items():
                        vf_src = x1[source_index_pair] - x_t[source_index_pair]
                        vf_src = vf_src / (1 - t)
                        # Where the sources are not being solved, we'll consider a velocity field
                        # of zero, so that those examples don't change in the solution.
                        vf_src[data["avail"] != 0] = 0
                        vf[source_index_pair] = vf_src
                ...
                # If predicting residuals, the solution of the ODE is the residual
                # between the predicted mean and the actual sample.
                if self.use_det_model and self.predict_residuals:
    ...
    def training_step(self, batch, batch_idx):
        ...
        # If using regularization, make a second pass with an unconditional batch
        self.reg_lambda = self.reg_lambda_max * max(
            self.reg_lambda_min, 1 - self.current_epoch / self.reg_max_epochs
        )  # Linearly decrease the regularization weight to the minimum value.
        if self.reg_lambda > 0.0:
            with torch.no_grad():
                uncond_x = self.to_unconditional_batch(masked_x)
                uncond_pred = self.forward(uncond_x)
        else:
            uncond_pred = None
        # Compute the loss
        loss = self.compute_loss(pred, batch, masked_x, path_samples, uncond_pred=uncond_pred)
...
def load_model(checkpoint_path):
    ...
    Returns:
        model (MultisourceFlowMatchingRegularizedDenoiser): The loaded model.
    """
    return MultisourceFlowMatchingDenoiser.load_from_checkpoint(checkpoint_path)
