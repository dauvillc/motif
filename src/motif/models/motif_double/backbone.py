...
class MultisourceGeneralBackbone(nn.Module):
    ...
    def __init__(
        ...
        if downsample_input:
            self.input_downsampling = MultiSourcePatchMerging(values_dim, coords_dim, cond_dim)
            values_dim *= 2
            coords_dim *= 2
            cond_dim *= 2
            self.input_upsampling = MultiSourcePatchSplitting(values_dim)
    ...
    def forward(self, x):
        ...
        if hasattr(self, "input_upsampling"):
            ...
            for source_index_pair in x.keys():
                ...
                x[source_index_pair]["values"] = x[source_index_pair]["values"][
